...
import { WeaponEntry, UpdateWeaponState, updateWeapon } from '@/data/character/weapons';
import { useActionState } from 'react';
import { FormInput } from '@/components/form/FormInput';
import { Form } from '../form';

type WeaponEditDialogProps = {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  weapon: WeaponEntry | null;
  onUpdated: (weapon: WeaponEntry) => void;
};

export const WeaponEditDialog = ({
  open,
  onOpenChange,
  weapon,
  onUpdated,
}: WeaponEditDialogProps) => {
  const [draft, setDraft] = useState<WeaponEntry | null>(weapon);
  ...
  useEffect(() => {
    if (state.status === 'success' && state.weapon) {
      onUpdated(state.weapon);
      onOpenChange(false);
    }
  }, [state, onUpdated, onOpenChange]);

  if (!draft || !weapon) return null;

  const handleText =
    (key: keyof WeaponEntry) => (e: ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) => {
      setDraft({ ...draft, [key]: e.target.value });
    };

  const handleSubmit = () => {
    const fd = new FormData();
    fd.set('characterItemId', weapon.id);
  ...
  return (
    <Dialog.Root open={open} onOpenChange={onOpenChange}>
      <Dialog.Content maxWidth="640px">
        <Dialog.Title>Edit weapon</Dialog.Title>
        <Dialog.Description size="2" mb="3">
          Update how this item is stored and displayed on your sheet.
        </Dialog.Description>

        <Form showActions={false}>
          <Box mb="3">
            <Heading size="4">{weapon.name}</Heading>
          ...
          {state.status === 'error' && (
            <Text color="red" size="2" mt="2">
              {state.message ?? 'Failed to update weapon.'}
            </Text>
          )}
